...
log_filename = "advtrain_joint_img_Ftext.txt"
...
# --- Text PGD Config (embedding-space budget) ---
TXT_EPS   = 0.05
TXT_ALPHA = 0.01
TXT_STEPS = 5

# Evaluation attack steps
EVAL_STEPS = 10
...
# Optional: freeze toggles
FREEZE_VISION = False   # set True if you want to freeze vision encoder
FREEZE_TEXT   = True    # set True if you want to freeze text encoder
...
# CLIP preprocessing normalization stats (preprocess already normalizes to these)
...
if FREEZE_TEXT:
    print("🔒 Freezing CLIP Text Encoder...")
    ...
    # text_projection is typically already in model.parameters(); freezing above covers it
    # but we enforce explicitly if present as a Parameter
...
# ----------------------------
# 4. Model (supports adv text embeddings)
# ----------------------------
class CLIPBinaryClassifier(nn.Module):
    ...
    def encode_text_from_embeddings(self, text_embeds, text_tokens):
        # replicates CLIP.encode_text but with injected embeddings
...
# ----------------------------
# 6. Joint adversarial example generation (Image + Text)
# ----------------------------
def clamp_clip_normalized(x_norm):
    return torch.max(torch.min(x_norm, CLIP_NORM_MAX), CLIP_NORM_MIN)

def gen_adv_joint(model, images_norm, text_tokens, labels, criterion,
                  img_eps, img_alpha, img_steps,
                  txt_eps, txt_alpha, txt_steps):
    """
    images_norm: CLIP-normalized images [B, N, 3, H, W]
    - Image PGD is done in normalized space, with img_eps/img_alpha specified in pixel space.
    - Text PGD is done in embedding space by learning delta_txt.
    """
    model.eval()

    # Convert pixel-space budgets to normalized-space budgets per channel
    img_eps_norm   = (img_eps / CLIP_STD)
    img_alpha_norm = (img_alpha / CLIP_STD)

    # Adversarial image variable
    ...
    for step in range(joint_steps):
        ...
        grad_img, grad_txt = torch.autograd.grad(loss, [adv_images, delta_txt], retain_graph=False, create_graph=False)
...
print("\nStarting Adversarial Training (Clean + Adv; joint Img+Txt PGD)...")
...
for epoch in range(epochs):
    ...
    for texts, imgs, labels in progress_bar:
        ...
        imgs = imgs.to(device, non_blocking=True)      # clean normalized images
        labels = labels.to(device, non_blocking=True)

        # 1) Create adversarial examples (perturb BOTH)
        adv_imgs, adv_text_embed = gen_adv_joint(
...
def evaluate_robustness_joint(model, val_loader, device,
                              ...
                              txt_eps, txt_alpha):
    print("\n--- Final Robustness Evaluation (Joint Img+Txt PGD) ---")
    model.eval()
...
def plot_clean_vs_adv_roc(clean_probs, adv_probs, labels, save_path="roc_clean_vs_adv_joint_Ftext.png"):
